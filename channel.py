...
class Channels:
    ...
    def Rayleigh(self, Tx_sig, SNR):
        ...
        H_real = torch.normal(0, math.sqrt(1/2), size=[batch_size, 2], device=Tx_sig.device)
        H_imag = torch.normal(0, math.sqrt(1/2), size=[batch_size, 2], device=Tx_sig.device)
        H = torch.zeros(batch_size, 2, 2, device=Tx_sig.device)
        H[:, 0, 0] = H_real[:, 0]
        H[:, 0, 1] = -H_imag[:, 0]
        H[:, 1, 0] = H_imag[:, 0]
        H[:, 1, 1] = H_real[:, 0]  # Or H_real[:, 1] for more diversity

        # Apply channel
        Tx_sig_reshaped = Tx_sig.view(batch_size, num_pairs, 2)
        Tx_sig_faded = torch.bmm(Tx_sig_reshaped, H)
        Tx_sig_faded = Tx_sig_faded.view(shape)

        return self.AWGN(Tx_sig_faded, SNR)
